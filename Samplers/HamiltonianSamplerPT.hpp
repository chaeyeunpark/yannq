...
public:
	HamiltonianSamplerPT(const Machine& qs, const Hamiltonian& ham, int numChain)
		: n_(qs.getN()), numChain_(numChain), qs_(qs), ham_(ham)
	{
...
#pragma omp for schedule(static, 4)
			for(int cidx = 0; cidx < numChain_; cidx++)
			{
				for(int sidx = 0; sidx < n_; sidx++)
				{
					...
					double p = std::min(1.0,exp(2.0*betas_[cidx]*real(sv_[cidx].logRatio(toFlip)))*w1/w2);
